...
Optional local reverse-proxy deployment evidence can be collected with:

```sh
script/bench-hypnotoad --reverse-proxy --proxy auto --check \
  --profile small --workers 2 --iterations 2 --warmup 1
```

This command requires the configured PostgreSQL benchmark database and a local
`nginx` or `haproxy` binary. If no requested proxy binary is available, the
command fails explicitly instead of reporting a false pass. The command surface
can be checked without live services with:

```sh
script/bench-hypnotoad --dry-run --reverse-proxy --json \
  --iterations 1 --warmup 0 --route /health/live
```
...
| Command | Result |
| --- | --- |
| `script/gpforum-carton exec prove -lr t` | passing |
...
| Hypnotoad reverse-proxy evidence dry-run | passing with `script/bench-hypnotoad --dry-run --reverse-proxy --json`; live evidence supports local Nginx or HAProxy in front of temporary Hypnotoad |
...
Latest full test suite after reverse-proxy evidence and MVP flow hardening:

```text
Files=61, Tests=3003, Result=PASS
```
...
The repository currently has 61 `.t` files covering:
...
* local cache, benchmark harness, reverse-proxy deployment evidence, MVP user
  flow smoke coverage, query-plan gate and browser security headers.
...
* Reverse-proxy deployment evidence now starts temporary Nginx or HAProxy in
  front of Hypnotoad, records frontend/backend ports, proxy version, worker
  count, query-budget observations and direct-Hypnotoad comparison.
...
* Archive live reverse-proxy benchmark JSON as CI artifacts once runners can
  install an optional local Nginx or HAProxy package.
...
1. Archive medium/hot-thread JSON evidence in CI artifacts.
2. Add longer soak checks for worker RSS drift on hot-thread rendering.
3. Keep reverse-proxy evidence fresh across the locally available proxy.
