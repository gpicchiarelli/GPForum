...
| PRIVATE BETA READY | no | Harness + Cloud Agent VM drill archive in `docs/ops/evidence/2026-09-20-cloud-agent-drills/` (`carton_ok`; staging-host-verify / staging-drill / attachments / mail dry-run pass; stress smoke ok). Restano evidenza su **staging TLS reale** (SMTP `--send`, stress 100+, deploy/systemd install target). **PRIVATE BETA NOT YET.** |
