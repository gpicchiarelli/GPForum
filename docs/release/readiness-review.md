...
| Audit trail integrity | PARTIAL | `record_hash` canonico e chain locale verificati | Chain non serializzata sotto concorrenza, branching possibile | `audit_chain_state` con lock o advisory lock |
...
| --- | --- | --- |
| `command_log` race concorrente | Documentato, non chiuso | Upsert/catch unique e test PostgreSQL concorrente |
| Bookmark/subscription check-then-insert | Documentato, non chiuso | Upsert atomico e test concorrenti |
| Report duplicati aperti | Documentato, non chiuso | Unique partial index e replay controllato |
| Moderation actions senza command-id/row lock uniforme | Documentato, non chiuso | `command_id`, `FOR UPDATE`, audit/event single-write |
| Failure mode DB down/timeout/write after commit | Parziale | Test end-to-end su write principali |
| Audit chain non serializzata | Record hash presente, chain concorrente non garantita | `audit_chain_state` con lock e test concorrente |
