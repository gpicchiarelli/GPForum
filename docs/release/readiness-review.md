...
| PRIVATE BETA READY | no | Mail/moderation/idempotenza e evidenza PG a due connessioni (concurrency, idempotency, outbox reclaim) sono in codice; staging-drill DB è shippato. Restano attachment restore, deploy nginx/systemd end-to-end, SMTP staging e stress 100+. Può reggere solo una alpha privata operator-assisted. |
| PUBLIC PRODUCTION READY | no | Mancano staging rappresentativo end-to-end, stress test 100/500/1000 utenti, attachment restore drill e runbook di rollback provato sul target. |
...
| Stress test rappresentativo non eseguito | Local hot-thread smoke non prova 100/500/1000 utenti; harness sibling in corso | Eseguire load test staging con p50/p95/p99, error rate, worker distribution, DB latency |
| Backup/restore non provato su staging con attachment storage | DB dump/restore drill sì; blob `var/attachments` no | Drill restore completo DB + allegati + readiness (sibling attachment work in corso) |
