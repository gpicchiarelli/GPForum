...
### Upgrade note: preflight wrapper renamed

The wrapper is now `script/os-preflight`; it was `script/gpforum-os-preflight`.
The `ExecStartPre=` lines in `deploy/systemd/gpforum.service` and
`deploy/systemd/gpforum-unix-socket.service`, and the preflight call in
`deploy/freebsd/gpforum`, were updated to match.

**Existing installs must re-copy the unit files.** A host still running the
old units fails to start after upgrading, because `ExecStartPre=` points at a
path that no longer exists. Re-copy the unit files, then `systemctl
daemon-reload` (Linux) or reinstall the rc.d script (FreeBSD).
...
```sh
script/mail-check --human --dry-run
script/mail-check --send --to you@example.test --human
```
...
before stay served until the hourly backfill has scanned them. Install and start the system
package before the first start — `apt install clamav-daemon
clamav-freshclam`, `pkg install clamav`, or `port install clamav
clamav-server -scan_schedule_access -sanesecurity` — then prove it detects
the EICAR test file:
...
Set `StreamMaxLength 26M` in `clamd.conf`, and make sure the `gpforum` user
can connect to its socket. To run without an antivirus, set
`GPFORUM_ANTIVIRUS=none` explicitly. See `docs/ops/antivirus.md`.
...
- put the service's environment -- `GPFORUM_DATABASE_DSN`,
  ...
  is the only way the configuration reaches the application;
