...
| --- | --- |
| Web | `bin/gpforum`, `GPForum.pm`, `Bootstrap::*`, `Controller::*`, SSR templates, `Web::*Payload`, `Web::Guard`, `Web::RenderPolicy`, `Web::PublicHttpCache` |
...
| Infra | `deploy/nginx`, `deploy/systemd`, `deploy/freebsd`, `deploy/launchd`, `script/os-preflight`, runtime policy classes |
| Observability | `/health/live`, `/health/ready`, `/metrics`, DB query observer, query budget gates, query plan evidence, benchmarks |

Entrypoints:

- Web: `script/gpforum-carton exec hypnotoad -f bin/gpforum`
- Development web: `script/gpforum-carton exec morbo bin/gpforum`
- Migration: `script/gpforum-carton exec bin/gpforum-migrate --apply`
- Outbox worker: `script/gpforum-carton exec bin/gpforum-outbox-dispatch --loop --limit 100 --sleep 5`
...
- Admin bootstrap: `script/gpforum-carton exec bin/gpforum-admin-bootstrap --user-id USER_ID`
...
- Controllers still contain shared HTTP helpers in `Web::*` and per-boundary
  bases. Forum, identity, moderation, and admin writes now live in dedicated
  controllers and workflow services. Do not expand remaining controllers
  before moving more shared auth/CSRF/error helpers into `Web::*`.
...
- OS system Perl `5.38.0` or newer (`/usr/bin/perl` / distro package, or
  MacPorts `/opt/local/bin/perl`; version managers and custom PREFIX builds
  are unsupported). Verify with `make system-perl`. On macOS MacPorts hosts,
  `eval "$(script/gpforum-macports-env)"` puts PostgreSQL client tools on
  `PATH`.
...
script/gpforum-carton exec bin/gpforum-migrate --plan
script/gpforum-carton exec bin/gpforum-migrate --apply
script/query-budget --sync
script/query-budget --check
script/gpforum-carton exec bin/gpforum-admin-bootstrap --user-id USER_ID
```
...
script/query-budget --check
script/os-preflight --strict --json
```
...
script/gpforum-carton exec bin/gpforum-platform-check --with-db
```
...
```sh
script/gpforum-carton exec prove -lr t/13-outbox-dispatcher.t \
...
make install-deps-postgres
script/gpforum-carton exec bin/gpforum-migrate --apply
script/gpforum-carton exec morbo bin/gpforum
```
...
2. Install dependencies with `make install-deps-postgres`
   (`carton install --deployment` from `cpanfile.snapshot`).
3. Run `script/gpforum-carton check`.
4. Run `script/os-preflight --strict --json`.
