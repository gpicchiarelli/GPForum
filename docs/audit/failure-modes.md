...
| FM-009 | Command log race | vincolo unique, ma insert race non gestita atomicamente | retry può produrre 500/in-progress non stabile | high | upsert/catch unique e test concorrente su `command_log` |
| FM-010 | Audit hash-chain branching | record hash presente, chain non serializzata | audit non lineare sotto concorrenza | high | chain state con `FOR UPDATE` o advisory lock e test concorrente |
...
1. `command_log` concorrente: due connessioni con stesso `command_id`, un solo
   comando registrato, seconda risposta replay o `in_progress`, mai 500.
2. Report duplicato concorrente: unique partial su report aperti e retry stabile.
3. Worker crash: dispatch riuscito, crash prima di mark done, reclaim dopo
   `locked_until`, handler idempotente.
4. Audit chain: due audit append concorrenti devono produrre chain lineare.
5. Errore dopo commit HTTP: simulare risposta persa e retry client sulle write
   principali.
