...
- Reply hot path: `PostStore` assigns the position inside the transaction, locks
  the thread with `FOR NO KEY UPDATE` (ADR 0111), re-checks that the thread is
  still open under that lock, and the DB constraint `(thread_id, position)`
...
`deleted_at` né `note`.
Test fake in `t/159-concurrency-correctness.t` e `t/24-advanced-community.t`;
...
`t/159-concurrency-correctness.t` e `t/17-notifications.t`; evidenza PG in
`t/integration/postgres-concurrency.t`.
...
`t/159-concurrency-correctness.t`; evidenza PG in
`t/integration/postgres-concurrency.t`.
...
`t/159-concurrency-correctness.t`, `t/25-moderation-review.t` e
...
- `lib/GPForum/Service/Outbox/HandlerIdempotency.pm`
