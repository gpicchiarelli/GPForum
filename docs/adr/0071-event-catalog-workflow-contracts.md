...
- `search.rebuild_requested`
  - producer: the console's search rebuild (`Admin::Maintenance` via
    `Search::RebuildRun`), then each step for the next
  - consumers: search indexing (one batch per event)
  - payload: `run_id`, `entity_type`, `stage`, `after`, `step`, `totals`
  - idempotency: `search.rebuild_requested:<run>:<stage>:<after>`, so a
    retried step never records its successor twice
- `search.rebuild_completed`
  - producer: the last step of a run
  - consumers: the console's search status
  - payload: `run_id`, `entity_type`, `step`, `totals`
