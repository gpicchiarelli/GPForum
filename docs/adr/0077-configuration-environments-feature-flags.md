...
- Open conflicts: `GPForum::Config` defines no keys for object storage, email
  provider, PostgreSQL search configuration or security mode. The `etc/*.conf`
  profiles named below were never read by anything and have been removed; the
  operational profile values live in the `%PROFILES` constant in
  `GPForum::Service::Operations::Profile`. Attachment storage was hard-wired to
  `var/attachments`; it is now `GPFORUM_ATTACHMENT_ROOT`.
...
- Code: `lib/GPForum/Config.pm`, `lib/GPForum/Runtime.pm`,
  `lib/GPForum/Service/Operations/Profile.pm`.
