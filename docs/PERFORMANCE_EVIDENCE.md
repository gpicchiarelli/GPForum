...
It executes `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)` over the statement each
endpoint really runs. Nothing is transcribed: the SQL is rendered from the
resultset the named method returns -- the same method the application calls --
or, for the outbox claim, taken from the claim query itself. Changing a reader's
query changes what the gate EXPLAINs.

| Endpoint | Query label | Source |
| --- | --- | --- |
| home | `threads_public_activity` | `ThreadReader::latest_threads_resultset` |
| categories | `categories_position` | `CategoryReader::categories_resultset` |
| category_threads | `threads_category_activity_visible_locked` | `ThreadReader::category_threads_resultset`, anonymous |
| category_threads_signed_in | `threads_category_viewer_union` | the same, with a viewer |
| thread_view | `posts_visible_thread_position` | `PostReader::thread_posts_resultset` |
| search | `search_documents_vector` | `Searcher::search_resultset` |
...
Each plan is taken inside a transaction that is rolled back: `EXPLAIN ANALYZE`
executes the statement, and the outbox claim is an `UPDATE`.

Failure rules are intentionally conservative:

| Risk | Rule |
| --- | --- |
| No usable index | fail when a sequential scan survives with sequential scans disabled, at any table size, unless the table is small by construction |
...
The first rule is the one that works on a test-sized database. The others read
row counts, and on a seed where every table is small every plan is small. It is
a floor, not a proof of the best plan: it catches a query no index can answer,
not a query answered by the wrong index.
