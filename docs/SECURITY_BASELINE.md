...
```sh
script/gpforum-carton exec prove -lr t/06-identity-web.t t/43-moderation-web.t t/44-admin-web.t t/48-browser-security.t t/50-security-hardening.t
...
`GPForum::Service::Identity::Event` owns login/logout AuditLog hashes.
`GPForum::Service::Identity::SecurityAudit` records those rows without
storing raw identifiers or request addresses. Login identifiers and request
addresses are SHA-256 hashed before entering audit metadata.
