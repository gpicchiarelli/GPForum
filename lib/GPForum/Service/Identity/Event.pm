...
sub registered_envelope ( $, $user, $correlation_id ) {
    return {
        actor_id          => $user->{id},
...
sub registered_audit ( $, $user, $correlation_id ) {
    return {
        action         => $REGISTERED,
        actor_id       => $user->{id},
        correlation_id => $correlation_id,
...
sub logout_request_audit ( $, $input ) {
    return {
