...
sub _ensure_role_permissions {
    ...
    for my $permission_definition ( @{ $input->{permissions} } ) {
        my $permission = $self->_ensure_permission($permission_definition);
