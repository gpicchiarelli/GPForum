...
sub _outbox_event_mapper ($self) {
    my $mapper = $self->outbox_event_mapper;
    if ( $mapper->can('schema') && !$mapper->schema ) {
        $mapper->schema( $self->schema );
    }

    # Badges count what the inbox shows, judged as the hub judges its
    # thread channels (ADR 0102).
    if (   $mapper->can('readability')
        && !$mapper->readability
        && $self->hub
        && $self->hub->can('readability') )
    {
        $mapper->readability( $self->hub->readability );
    }
