...
sub visibility_allows {
    my ( $self, $target, $attachment, $viewer_user_id ) = @_;
...
sub _linked_target_allows {
    ...
    return $self->visibility_allows( $input->{target}, $input->{attachment},
        $input->{viewer_user_id},
    );
}
...
sub _open_visibility {
    my ( $self, $visibility, $viewer_user_id ) = @_;
...
sub _private_author {
    my ( $self, $visibility, $target, $viewer_user_id ) = @_;
