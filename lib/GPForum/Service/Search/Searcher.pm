...
const my $RANK_EXPRESSION => q{
    greatest(
        ts_rank_cd(search_vector, websearch_to_tsquery(language, ?), 32),
        similarity(title_normalized, lower(?)) * 0.2
    )
};
const my $FTS_CONDITION =>
  q{search_vector @@ websearch_to_tsquery(language, ?)};
const my $TRIGRAM_CONDITION => q{similarity(title_normalized, lower(?)) >= ?};
const my $TITLE_CONTAINS_CONDITION =>
  q{title_normalized LIKE lower(?) ESCAPE '\'};
    ...
    my $search     = $self->schema->resultset('SearchDocument')->search(
        {
            ...
            title_normalized => { -like => _like_pattern($normalized) . q{%} },
        },
        {
            join      => 'author',
...
sub _search_query {
    ...
    $where->{author_user_id} = $options->{author_user_id}
      if _defined_non_empty( $options->{author_user_id} );

    my @date_filters;
    push @date_filters, { source_created_at => { '>=' => $options->{from} } }
      if _defined_non_empty( $options->{from} );
    push @date_filters, { source_created_at => { '<=' => $options->{to} } }
