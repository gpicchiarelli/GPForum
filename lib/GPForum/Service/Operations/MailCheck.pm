...
sub _run_configured ( $self, $config, $options, $mode ) {
...
sub _config_fail ( $, $mode, $summary, $error ) {
    return {
        status => $STATUS_FAIL,
        check  => 'mail_delivery',
        mode   => $mode,
...
sub _human_lines ( $self, $evidence ) {
    my @lines = (
...
sub _human_smtp_lines ( $, $evidence ) {
    my $config = $evidence->{config} // {};
...
sub _human_probe_lines ( $, $evidence ) {
