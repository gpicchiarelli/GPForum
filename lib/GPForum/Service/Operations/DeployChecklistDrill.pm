...
sub _verify_one_unit ( $root, $workspace, $binary, $check ) {
...
sub _nginx_test_one ( $root, $workspace, $binary, $check ) {
    my $source = path( $root, $check->{path} )->to_string;
...
sub _render_nginx_site ( $source, $destination, $workspace ) {
    my $text   = path($source)->slurp;
...
sub _host_status ( $systemd, $nginx ) {
...
sub _static_groups_pass ($checklist) {
