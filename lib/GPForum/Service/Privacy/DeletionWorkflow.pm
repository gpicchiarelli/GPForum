...
sub _create_or_reuse_deletion ( $self, $input ) {
...
sub _insert_or_reuse_deletion ( $self, $input ) {
    my ( $created, $error ) =
      GPForum::Infrastructure::UniqueConflict->attempt( $self->schema,
        sub { return $self->_create_deletion_request($input); },
      );
    if ($created) {
        return $created;
    }
...
sub _deletion_after_unique ( $self, $input, $error ) {
...
sub _retry_deletion_id ( $self, $input ) {
    my ( $created, $error ) =
      GPForum::Infrastructure::UniqueConflict->attempt( $self->schema,
        sub { return $self->_create_deletion_request($input); },
      );
...
sub _deletion_id_conflict ($error) {
...
sub _open_deletion_conflict ($error) {
    if ( !defined $error || !length $error ) {
        return 0;
    }
...
sub _create_deletion_request ( $self, $input ) {
    my $request = {
        completed_at        => undef,
...
sub _deletion_event_exists ( $self, $existing ) {
    return $self->recorder->event_recorded( join q{:},
        'privacy.deletion_requested', $existing->{deletion_request_id} );
}

sub _approve_in_txn ( $self, $input ) {
...
sub _approved_or_held ( $self, $input ) {
...
sub _hold_or_create ( $self, $input ) {
...
sub _create_approval ( $self, $input ) {
...
sub _existing_approval_action ( $self, $request_id ) {
...
sub _approval_action ( $self, $input ) {
...
sub _create_erasure_job ( $self, $input ) {
    my $job = {
        completed_at        => undef,
...
sub _job_after_unique ( $self, $input, $error ) {
...
sub _retry_erasure_id ( $self, $input ) {
    my ( $created, $error ) =
      GPForum::Infrastructure::UniqueConflict->attempt( $self->schema,
        sub { return $self->_create_erasure_job($input); },
      );
...
sub _erasure_id_conflict ($error) {
    if ( !defined $error || !length $error ) {
        return 0;
    }
...
sub _erasure_request_conflict ($error) {
    if ( !defined $error || !length $error ) {
        return 0;
    }
...
sub _complete_in_txn ( $self, $input ) {
    my $job =
...
sub _job_completed_at ( $self, $job ) {
...
sub _complete_with_request ( $self, $input, $job ) {
...
sub _erase_or_block ( $self, $input ) {
    my $hold = $self->_active_hold_for_request( $input->{request} );
    if ($hold) {
        $input->{hold} = $hold;
...
sub _block_or_replay ( $self, $input ) {
...
sub _complete_action ( $self, $input, $anonymized ) {
    return $self->_record_action(
        $self->record->column( $input->{request}, 'deletion_request_id' ),
        {
...
sub _lock_deletion_request_for_approval ( $self, $request_id ) {
...
sub _lock_open_deletion ( $self, $input ) {
    my $dbh = $self->_schema_dbh;
    if ( !$dbh ) {
        return;
    }
...
sub _open_deletion_query ( $, $input ) {
    return {
        request_type  => $input->{request_type},
        resource_id   => $input->{resource_id},
        resource_type => $input->{resource_type},
...
sub _record_action ( $self, $request_id, $input ) {
    my ( $created, $error ) =
      GPForum::Infrastructure::UniqueConflict->attempt( $self->schema,
        sub { return $self->_insert_deletion_action( $request_id, $input ); },
      );
    if ($created) {
        return $created;
    }
...
sub _insert_deletion_action ( $self, $request_id, $input ) {
    my $action = {
...
sub _deletion_action_after_conflict ( $self, $request_id, $input, $error ) {
    if ( !GPForum::Infrastructure::UniqueConflict->is_conflict($error) ) {
        GPForum::Infrastructure::UniqueConflict->rethrow($error);
    }
...
sub _retry_deletion_action_id ( $self, $request_id, $input ) {
    my ( $created, $error ) =
      GPForum::Infrastructure::UniqueConflict->attempt( $self->schema,
        sub { return $self->_insert_deletion_action( $request_id, $input ); },
      );
...
sub _job_has_block_error ( $self, $job ) {
...
sub _hold_action ( $self, $input, $request_id ) {
    return $self->_record_action(
        $request_id,
        {
            action_type => 'held',
...
sub _erase_user ( $self, $user, $user_id, $timestamp ) {
