...
sub envelope ( $self, $input, $correlation_id ) {
    my $request = $input->{request};
...
sub audit ( $self, $input, $correlation_id ) {
    my $request = $input->{request};
...
sub _payload_with ( $self, $request, $extra ) {
    return { %{ $self->record->request_payload($request) }, %{$extra}, };
}

sub _job_id ( $self, $job ) {
