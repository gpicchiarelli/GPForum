...
const my $ACCOUNT_LOGIN_LIMIT       => 20;
...
const my %ACTION_LIMIT_FOR => (
    'identity.login_account'   => $ACCOUNT_LOGIN_LIMIT,
...
sub profile_thread_limit ( $, $requested ) {
...
sub preference_cookie_options ( $, $now ) {
    return {
...
sub bad_request ( $self, $controller ) {
    return $self->_json_or_text(
        $controller,
        {
...
sub _json_or_text ( $self, $controller, $input ) {
...
including verification, 20 for the logins one account receives from every
address together, and 10 for other identity writes including login.
