...
sub _enabled_option ( $requested, $supported ) {
    return $requested && $supported ? 1 : 0;
}

sub _degraded_option ( $requested, $supported ) {
    return $requested && !$supported ? 1 : 0;
}

sub _feature_enabled ( $features, $name ) {
...
sub _feature_setting ( $features, $name ) {
