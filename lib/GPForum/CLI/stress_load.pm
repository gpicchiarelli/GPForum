...
# work stays in GPForum::Command::StressLoad; this makes it discoverable.
has description => 'Generate stress load against a running instance';
has usage       => sub ($self) {
    return GPForum::Command::StressLoad->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::StressLoad->new->run(@arguments) );
}
