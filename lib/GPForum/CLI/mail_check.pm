...
package GPForum::CLI::mail_check;
## use critic
...
# work stays in GPForum::Command::MailCheck; this makes it discoverable.
has description => 'Send a test message through the configured mailer';
has usage       => sub ($self) {
    return GPForum::Command::MailCheck->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::MailCheck->new->run(@arguments) );
}
