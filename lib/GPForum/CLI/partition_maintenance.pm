...
# work stays in GPForum::Command::PartitionMaintenance; this makes it discoverable.
has description => 'Create the next log partitions before rows reach DEFAULT';
has usage       => sub ($self) {
    return GPForum::Command::PartitionMaintenance->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::PartitionMaintenance->new->run(@arguments) );
}
