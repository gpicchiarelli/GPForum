...
package GPForum::CLI::outbox_dispatch;
## use critic
...
# work stays in GPForum::Command::OutboxDispatch; this makes it discoverable.
has description => 'Dispatch pending outbox messages';
has usage       => sub ($self) {
    return GPForum::Command::OutboxDispatch->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::OutboxDispatch->new( app => $self->app )
          ->run(@arguments) );
}
