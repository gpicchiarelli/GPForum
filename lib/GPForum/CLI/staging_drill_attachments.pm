...
package GPForum::CLI::staging_drill_attachments;
## use critic
...
# work stays in GPForum::Command::StagingDrillAttachments; this makes it discoverable.
has description => 'Run the attachment store drill';
has usage       => sub ($self) {
    return GPForum::Command::StagingDrillAttachments->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::StagingDrillAttachments->new->run(@arguments) );
}
