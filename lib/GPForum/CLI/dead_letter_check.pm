...
# work stays in GPForum::Command::DeadLetterCheck; this makes it discoverable.
has description => 'Report exhausted outbox messages';
has usage       => sub ($self) {
    return GPForum::Command::DeadLetterCheck->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::DeadLetterCheck->new->run(@arguments) );
}
