...
package GPForum::CLI::scheduled_jobs;
## use critic
...
# work stays in GPForum::Command::ScheduledJobs; this makes it discoverable.
has description => 'Run the retention and cleanup jobs once';
has usage       => sub ($self) {
    return GPForum::Command::ScheduledJobs->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::ScheduledJobs->new( app => $self->app )
          ->run(@arguments) );
}
