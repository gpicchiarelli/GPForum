...
# work stays in GPForum::Command::Migrate; this makes it discoverable.
has description => 'Apply the SQL migrations and record their checksums';
has usage       => sub ($self) {
    return GPForum::Command::Migrate->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::Migrate->new->run(@arguments) );
}
