...
package GPForum::CLI::search_rebuild;
## use critic
...
# work stays in GPForum::Command::SearchRebuild; this makes it discoverable.
has description => 'Rebuild the search index, or report its lag';
has usage       => sub ($self) {
    return GPForum::Command::SearchRebuild->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::SearchRebuild->new->run(@arguments) );
}
