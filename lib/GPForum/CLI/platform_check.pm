...
package GPForum::CLI::platform_check;
## use critic
...
# work stays in GPForum::Command::PlatformCheck; this makes it discoverable.
has description =>
  'Check the platform prerequisites, with or without a database';
has usage => sub ($self) {
    return GPForum::Command::PlatformCheck->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::PlatformCheck->new->run(@arguments) );
}
