...
package GPForum::CLI::os_preflight;
## use critic
...
# work stays in GPForum::Command::OsPreflight; this makes it discoverable.
has description => 'Report whether this host can run GPForum';
has usage       => sub ($self) {
    return GPForum::Command::OsPreflight->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::OsPreflight->new->run(@arguments) );
}
