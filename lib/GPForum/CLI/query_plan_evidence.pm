...
# work stays in GPForum::Command::QueryPlanEvidence; this makes it discoverable.
has description => 'Collect EXPLAIN evidence for the budgeted endpoints';
has usage       => sub ($self) {
    return GPForum::Command::QueryPlanEvidence->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::QueryPlanEvidence->new->run(@arguments) );
}
