...
# work stays in GPForum::Command::HypnotoadScaling; this makes it discoverable.
has description => 'Benchmark Hypnotoad across worker counts';
has usage       => sub ($self) {
    return GPForum::Command::HypnotoadScaling->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::HypnotoadScaling->new->run(@arguments) );
}
