...
package GPForum::CLI::query_budget;
## use critic
...
# work stays in GPForum::Command::QueryBudget; this makes it discoverable.
has description => 'Print, sync or check the query-plan budget catalog';
has usage       => sub ($self) {
    return GPForum::Command::QueryBudget->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::QueryBudget->new->run(@arguments) );
}
