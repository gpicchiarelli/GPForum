...
package GPForum::CLI::mail_lifecycle_check;
## use critic
...
# work stays in GPForum::Command::MailLifecycleCheck; this makes it discoverable.
has description => 'Exercise the mail lifecycle end to end';
has usage       => sub ($self) {
    return GPForum::Command::MailLifecycleCheck->usage_text;
};

sub run ( $self, @arguments ) {
    return GPForum::Command::Usage->front_door(
        GPForum::Command::MailLifecycleCheck->new->run(@arguments) );
}
