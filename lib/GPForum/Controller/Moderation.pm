...
sub release_report {
    ...
    my $released = eval {
        return $self->gp_report_store->release_report(
            $self->param('report_id'), $user_id );
    };

    return _system_failure($self)                  if $EVAL_ERROR;
    return _not_found( $self, 'report not found' ) if !$released;

    return _action_response( $self, 'released', $released );
}
