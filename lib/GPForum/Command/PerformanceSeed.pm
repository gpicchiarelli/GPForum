...
sub _insert_roles_and_permissions ( $dbh, $plan ) {
    _insert_roles($dbh);
...
sub _insert_role_permissions ($dbh) {
    my @role_permissions = (
...
sub _insert_role_bindings ( $dbh, $plan ) {
...
sub _insert_categories ( $dbh, $plan ) {
...
sub _insert_threads_and_posts ( $dbh, $plan ) {
...
sub _insert_thread ( $dbh, $plan, $thread_number ) {
    $dbh->do(
        q{
            INSERT INTO threads
...
sub _insert_thread_posts ( $dbh, $plan, $thread_number ) {
...
sub _insert_post_head ( $dbh, $plan, $thread_number, $position, $post_id ) {
    $dbh->do(
        q{
            INSERT INTO posts
...
sub _insert_post_body ( $dbh, $post_id, $body_id, $body, $position ) {
    $dbh->do(
        q{
...
sub _insert_post_revision ( $dbh, $plan, $post_id, $body_id, $revision_id ) {
    $dbh->do(
        q{
...
sub _update_post_current_version ( $dbh, $post_id, $body_id, $revision_id ) {
    $dbh->do(
        q{
            UPDATE posts
               SET current_body_id = ?,
...
sub _insert_thread_counter ( $dbh, $plan, $thread_number ) {
...
sub _insert_search_document ( $dbh, $plan, $thread_number ) {
...
sub _insert_category_stats ( $dbh, $plan ) {
    for my $number ( 1 .. $plan->{dataset}{categories} ) {
...
sub _insert_read_state_for_user ( $dbh, $plan, $user_number ) {
...
sub _upsert_read_state ( $dbh, $plan, $user_number, $thread_number ) {
...
sub _insert_bookmarks ( $dbh, $plan ) {
...
sub _insert_subscriptions ( $dbh, $plan ) {
...
sub _insert_notifications ( $dbh, $plan ) {
...
sub _insert_notification ( $dbh, $plan, $number ) {
...
sub _insert_feed_items ( $dbh, $plan ) {
    for my $thread_number ( 1 .. $plan->{dataset}{threads} ) {
        $dbh->do(
            q{
...
sub _insert_moderation_actions ( $dbh, $plan ) {
...
sub _permission_id ($number) {
    return _uuid( $FAMILY_PERMISSION, $number );
}

sub _thread_id ($number) {
...
sub _role_number_for_user ($user_number) {
...
sub _threads_in_category ( $plan, $category_number ) {
    my $threads = 0;
    for my $thread_number ( 1 .. $plan->{dataset}{threads} ) {
        $threads++
...
sub _post_body ( $thread_number, $position ) {
    return
        'Performance post '
      . $position
      . ' in thread '
