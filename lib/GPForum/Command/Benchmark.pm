...
sub _benchmark ( $self, $test, $options ) {
...
sub _warm_route ( $test, $route, $warmup ) {
    for ( 1 .. $warmup ) {
...
sub _db_query_status ($db_queries) {
...
sub _db_queries_observed ($db_queries) {
    return $db_queries && $db_queries->{observed} ? 1 : 0;
}

sub _db_query_failure ($db_queries) {
...
sub _db_duplicate_query_failure ($db_queries) {
...
sub _threshold_status ( $p95, $p99, $rps, $threshold ) {
    return 'fail'
...
sub _db_query_summary ( $observations, $budget ) {
...
sub _observed_budget_status ( $queries, $transactions, $budget ) {
    return 'none' if !$budget;
...
sub _compare_route_to_baseline ( $route, $baseline, $tolerance ) {
    if ( !$baseline ) {
        $route->{regression} = {
            status     => 'missing',
            violations => [
...
sub _add_upper_violation ( $violations, $route, $baseline, $tolerance, $metric )
{
...
sub _add_lower_violation ( $violations, $route, $baseline, $tolerance, $metric )
{
...
sub _regression_text ($regression) {
