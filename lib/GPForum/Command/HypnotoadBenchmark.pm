...
sub _reverse_proxy_report {
    my ( $backend_runtime, $proxy_runtime, $options, $direct_report ) = @_;
...
sub _start_reverse_proxy {
    my ( $backend_runtime, $options, $resolved_proxy ) = @_;
...
sub _write_reverse_proxy_config {
    my ($settings) = @_;
...
sub _reverse_proxy_command {
    my ( $kind, $binary, $config_file ) = @_;
...
sub _reverse_proxy_runtime_metadata {
    my ( $backend_runtime, $proxy_runtime, $options ) = @_;
...
sub _usage {
    return
'Usage: script/bench-hypnotoad [--json] [--check] [--dry-run] [--seed] [--profile small|medium|hot-thread] [--workers N] [--iterations N] [--warmup N] [--route /path] [--port N] [--reverse-proxy] [--proxy auto|nginx|haproxy] [--frontend-port N] [--no-compare] [--no-direct-compare] [--regression-tolerance N]';
}
