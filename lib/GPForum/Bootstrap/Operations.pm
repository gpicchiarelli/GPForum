...
sub _register_runtime_helpers ( $application, $config, $runtime,
    $runtime_policy )
{
...
sub _register_operational_helpers {
    ...
    $application->helper(
        gp_rate_limiter => sub {
            ...
            $rate_limiter ||= GPForum::Service::Operations::RateLimiter->new(
                degradation_policy => _rate_limit_degradation_policy($config),
                primary_store      =>
    ...
    $application->helper(
        gp_readiness => sub {
            ...
            # from_config returns nothing when scanning is off; scalar keeps that
            # an undef value instead of shifting every later pair out of place.
            return GPForum::Service::Operations::Readiness->new(
                antivirus =>
                  scalar GPForum::Infrastructure::Antivirus->from_config(
                    $config),
...
sub _application_cache ( $config, $controller ) {
    ...
    _attach_invalidation_bus( $cache, $controller );
...
sub _attach_invalidation_bus ( $cache, $controller ) {
    ...
    my $schema = eval { return $controller->gp_schema };
    if ( !$schema ) {
        return;
    }
...
# Development keeps the permissive local-memory fallback; staging and the
# production profiles fail closed so a database outage cannot lift the limit.
sub _rate_limit_degradation_policy ($config) {
    return
...
sub _safe_request_id ($request_id) {
...
sub _query_budget_endpoint ($controller) {
