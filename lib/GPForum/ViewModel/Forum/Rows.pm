...
sub thread ( $self, $row ) {
    ...
    return {
        ...
        category_title       => $self->loaded_column( $row, 'category_title' ),
        $self->_placement_visibility($row),
...
# The category's and space's visibility, when the reader loaded them (the
# thread page's does): page metadata then judges the effective visibility,
# not the thread's alone (ADR 0102). Left out otherwise, rather than undef,
# which would count as private.
sub _placement_visibility ( $self, $row ) {
    my %placement;
    for my $name (qw(category_visibility space_visibility)) {
        my $value = $self->loaded_column( $row, $name );
        if ( defined $value ) {
...
sub autocomplete_suggestion ( $self, $row ) {
    my $username = $self->column( $row, 'author_username' );
...
sub _stored_safe_body ( $self, $row ) {
