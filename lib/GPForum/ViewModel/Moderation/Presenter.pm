...
sub suspensions_page ( $self, %input ) {
    my $page = $input{page} || {};
...
sub report_action_response ( $self, $status, $report ) {
    return {
...
sub moderation_action ( $self, $result ) {
...
sub report ( $self, $row ) {
    ...
    return {
        ...
        lock_command_id    => $self->column( $row, 'lock_command_id' ),
