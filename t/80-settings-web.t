...
subtest 'settings persist locale theme and notification preferences' => sub {
    ...
    # 9.3: the member's zone is kept, shown selected, and used for dates.
    is( $identity_store->preferred_timezone,
        'Europe/Rome', 'settings update persists the time zone' );
    $test->element_exists(
        'select[name="timezone"] option[value="Europe/Rome"][selected]');
    $test->element_exists('select[name="timezone"] option[value=""]');

    my $again = _command_id_in_form( $test, '/settings' );
    $test->post_ok(
        '/settings' => form => {
            command_id => $again,
    ...
    is( $identity_store->preferred_timezone,
        undef, 'an unknown zone is stored as the forum default, not kept' );
    $test->get_ok('/settings');
    $test->element_exists('select[name="timezone"] option[value=""][selected]');
};

subtest 'the forum default zone is named in the settings form' => sub {
    my ( $test, undef ) = _authenticated_settings_app();
    $test->get_ok( '/settings' => { 'Accept-Language' => 'en' } );
    $test->text_is(
        'select[name="timezone"] option[value=""]' => 'Forum default (UTC)' );
};
