...
subtest 'permission denial stops before protected service helpers' => sub {
    my $test = _security_test_app();
    _install_test_session_route($test);
    $test->get_ok('/__test/session/user-normal');
    $test->status_is($HTTP_OK);
    ...
    for my $helper (
        qw(
        gp_role_catalog gp_permission_review gp_admin_audit_review
        gp_role_binding_store gp_report_store gp_moderation_review_reader
      ...
      )
    {
        $test->app->helper(
            $helper => sub {
                die "protected helper reached: $helper";
            }
        );
    }

    $test->get_ok( '/admin/roles' => { Accept => 'application/json' } );
    $test->status_is( $HTTP_FORBIDDEN,
        'denied admin read does not reach catalog service' );

    $test->post_ok(
        '/admin/users/user-2/roles' => { Accept => 'application/json' } =>
          form => {
            csrf_token    => $token,
    ...
    $test->status_is( $HTTP_FORBIDDEN,
        'denied admin write does not reach role binding service' );

    $test->get_ok( '/moderation/actions' => { Accept => 'application/json' } );
    $test->status_is( $HTTP_FORBIDDEN,
    ...
    $test->status_is( $HTTP_FORBIDDEN,
        'denied moderation release does not reach report service' );
};
...
sub _install_admin_fakes {
    ...
    $test->app->helper( gp_admin_console_reader => sub { return $services; } );
    $test->app->helper(
        gp_permission_gate => sub {
