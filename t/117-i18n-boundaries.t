...
# language_ranges is called in list context, so returning a scalar undef would
# yield a one-element list holding undef and make every request without an
# Accept-Language header — curl, health checks, most API clients — negotiate
# against an undefined range and warn.
for my $absent ( undef, q{} ) {
    my @warnings;
    local $SIG{__WARN__} = sub { push @warnings, $_[0] };
    my @ranges = $locales->language_ranges($absent);
    is( scalar @ranges,
        0,
        'an absent Accept-Language yields no ranges, not one undefined range' );
    is( $locales->negotiate($absent),
        'en', 'an absent Accept-Language negotiates the default locale' );
    is( scalar @warnings, 0, 'negotiating without a header warns nothing' );
}
