...
my $identity_base = GPForum::Controller::Identity::Base->new;
is( $identity_base->safe_return_to('/threads/thread-1'),
    '/threads/thread-1', 'safe_return_to keeps a relative path' );
is( $identity_base->safe_return_to('/settings?tab=email'),
    '/settings?tab=email', 'safe_return_to keeps a relative query string' );
for my $unsafe (
    q{},                          'https://evil.test/',
    '//evil.test/',               '/\\evil.test',
    '/threads\\evil.test',        "/threads\r\nSet-Cookie: a=b",
    "/threads\nevil",             "/threads\tevil",
    '/threads' . chr(0) . 'evil', "/threads\fevil",
  )
{
    is( $identity_base->safe_return_to($unsafe), q{/},
        'safe_return_to falls back to the site root for '
          . _printable($unsafe) );
}
...
sub _printable {
    ...
    if ( !length $value ) {
        return 'an empty return_to';
    }
    $value =~ s/([^[:print:]])/sprintf '\\x%02x', ord $1/msxge;
