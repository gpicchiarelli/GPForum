...
like(
    $local->{output},
    qr/operational_profile [ ] status=ok/msx,
    'local platform check prints operational profile status'
);
...
my $profile = _capture_stdout_status(
    sub {
        return $profile_command->run('--local');
    }
);
...
like(
    ...
    'local platform check reports operational profile failure'
);
