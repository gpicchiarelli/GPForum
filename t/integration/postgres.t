...
_profile_counts( $anonymous->app, $integration_database, $seeded );
...
# The profile counts came from a resultset passed straight into a helper,
# where DBIx::Class's list context handed over rows instead: the helper took
# the first row and counted nothing, and later died. Counted here against the
# database, with the same visibility rules, so the number has to be right.
sub _profile_counts {
    my ( $app, $database_info, $thread ) = @_;

    my $counts =
      GPForum::Service::Identity::ProfileReader->new(
        schema => $app->gp_schema )->public_profile( $thread->{username}, {} )
      ->{profile}{counts};
    my ($threads) = $database_info->{dbh}->selectrow_array(
        join( q{ },
            'SELECT count(*) FROM threads t JOIN users u',
            'ON u.id = t.author_user_id WHERE u.username = ?',
            'AND t.deleted_at IS NULL',
            q{AND t.moderation_state IN ('visible', 'locked')},
            q{AND t.visibility = 'public'} ),
        undef,
        $thread->{username}
    );

    cmp_ok( $threads, q{>}, 0, 'the seeded author has public threads' );
    is( $counts->{public_threads},
        $threads, 'the profile counts every public thread the author started' );
    is(
        $counts->{total_public},
        $counts->{public_threads} + $counts->{public_replies},
        'and the total is the sum of threads and replies'
    );
...
sub _create_thread {
    ...
    # The breadcrumb names the thread's category, read in the same statement
    # as the thread.
    $client->element_exists(qq{nav.breadcrumbs a[href="/c/$category_id"]});
    my $page  = $client->tx->res->dom;
    my $crumb = $page->at(qq{nav.breadcrumbs a[href="/c/$category_id"]});
    ok( $crumb && length $crumb->text, 'with the category title' );
...
sub _moderate_user {
    ...
    # A suspension is a destructive action: the form requires confirming it.
    _moderation_action(
        $client,
        "/moderation/users/$member_user_id/suspend",
        { confirm => 1 }
    );
...
sub _moderation_action {
    my ( $client, $path, $extra ) = @_;

    _post_form( $client, '/moderation/reports', $path,
        { reason => 'Integration moderation', %{ $extra || {} } } );
    _status_below( $client, $HTTP_BAD_REQUEST, "$path succeeds" );
