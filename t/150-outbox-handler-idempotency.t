...
# begin() is the claim. It writes a row so no other worker can take the same
# event, but the event is not done until the side effect has finished -- the
# distinction the single-timestamp row could not express.
ok( $store->begin($WORKER_SEARCH_KEY),         'begin claims an unheld key' );
ok( $schema->keys->rows->{$WORKER_SEARCH_KEY}, 'begin persists the claim row' );
ok( !$store->is_done($WORKER_SEARCH_KEY), 'a claimed key is not yet done' );

# The second worker is the whole point: it must be refused, because if it
# proceeded it would run the side effect a second time.
ok(
    !$store->begin($WORKER_SEARCH_KEY),
    'a second worker cannot claim a held key'
);

$store->mark_failed( $WORKER_SEARCH_KEY, 'boom' );
ok(
    !$store->is_done($WORKER_SEARCH_KEY),
    'mark_failed does not mark a worker key done'
);
ok( !$schema->keys->rows->{$WORKER_SEARCH_KEY},
    'mark_failed releases the claim so the event can be retried' );
ok( $store->begin($WORKER_SEARCH_KEY),
    'the released key can be claimed again' );
$store->mark_failed( $WORKER_SEARCH_KEY, 'boom' );

# A worker killed between claiming and finishing leaves its row behind. The
# claim is a lease, not a tombstone: once it is older than the lease another
# worker may take it, or the event would never run again.
ok(
    $store->begin($WORKER_SEARCH_KEY),
    'a key can be claimed for the lease test'
);
ok(
    !$store->begin($WORKER_SEARCH_KEY),
    'a claim inside its lease is not stealable'
);
$schema->keys->rows->{$WORKER_SEARCH_KEY}{created_at} = '2026-05-23T11:00:00Z';
ok( $store->begin($WORKER_SEARCH_KEY),
    'a claim older than the lease is taken over' );
$store->mark_failed( $WORKER_SEARCH_KEY, 'boom' );

$store->mark_done( $WORKER_SEARCH_KEY, { event_id => 'event-1' } );
ok( $store->is_done($WORKER_SEARCH_KEY), 'mark_done inserts the worker key' );

# A finished key must never be reclaimable, however old it is.
$schema->keys->rows->{$WORKER_SEARCH_KEY}{created_at} = '2026-05-23T09:00:00Z';
ok( !$store->begin($WORKER_SEARCH_KEY),
    'a completed key is never taken over, however old' );
