...
# A login does not go through the command log (a stored login was replayable
# without the password), so its outage does not stop members signing in.
$test->post_ok( '/login' => form => $login_form );
isnt( $test->tx->res->code,
    $HTTP_SERVICE_UNAVAIL, 'a login does not depend on the command log' );
$test->content_unlike($LEAK_PATTERN);
