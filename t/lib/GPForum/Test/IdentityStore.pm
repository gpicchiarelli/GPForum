...
has preferred_timezone => undef;
has timezone_updates   => sub { return []; };
...
sub update_preferred_timezone {
    my ( $self, $input ) = @_;

    my $zone = $input->{preferred_timezone};
    $self->preferred_timezone( defined $zone && length $zone ? $zone : undef );
    push @{ $self->timezone_updates }, { %{$input} };
