...
# The schema is optional: a resultset handed straight to a test, with no
# schema behind it, keeps answering as it always did.
sub _assert_usable {
    my ($self) = @_;
...
sub _matches_query {
    ...
    for my $field ( keys %{$query} ) {
        if ( $field eq '-or' ) {
            return if !any { _matches_query( $row, $_ ) } @{ $query->{-or} };
            next;
        }
