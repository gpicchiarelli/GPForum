...
# A job that failed makes the run fail: the summary says why, and the command
# exits 1 so the timer unit is marked failed rather than recording a clean run.
{
    my $fixture = GPForum::Test::AttachmentFixtures->build;
    my $failing = GPForum::Service::Operations::ScheduledJobs->new(
        attachment_scanner => GPForum::Service::Attachment::Scanner->new(
            antivirus => GPForum::Test::Antivirus->new( reachable => 0 ),
            storage   => $fixture->{storage},
            store     => $fixture->{store},
        ),
        attachment_store => $fixture->{store},
    );
    my $failed = $failing->run( { jobs => ['attachment_scans'] } );
    is( $failed->{ok}, 0, 'a failed job makes the run not ok' );
    ...
    my $exit = GPForum::Command::ScheduledJobs->new(
        jobs   => $failing,
        output => $handle,
    )->run( '--job', 'attachment_scans' );
    close $handle or croak 'close capture';
    is( $exit, 1, 'and the command exits 1' );
    like(
        $captured,
        qr/ok=0 .* attachment_scans_error="antivirus [ ] unavailable"/msx,
        'with the job\'s error on the summary line'
    );
}
