...
my $missing_login_command = $workflow->login(
    {
        identifier => 'giacomo',
        password   => 'secret-password',
    }
);
is( $missing_login_command->{status},
    'invalid', 'login rejects a missing command_id' );
is(
    $missing_login_command->{errors}{command_id},
    'command_id is required',
    'login names the missing command_id'
);
...
ok( $issued_login->{ok}, 'commanded login records a command' );
is( $idempotency->last_input->{command_type},
    'identity.login', 'login uses the identity.login command type' );
is_deeply(
    $idempotency->last_input->{request},
    { identifier => 'giacomo' },
    'login command log omits the password'
);

my $login_count    = _store_calls( $services, 'authenticate_login' );
my $replayed_login = GPForum::Service::Identity::Workflow->new(
    command_idempotency => GPForum::Test::CommandIdempotency->new(
...
is_deeply( $replayed_login, $issued_login,
    'login replays the recorded result' );
is( _store_calls( $services, 'authenticate_login' ),
    $login_count, 'login replay does not open a second session' );

my $conflicted_login = GPForum::Service::Identity::Workflow->new(
    command_idempotency => GPForum::Test::CommandIdempotency->new(
        conflict => 1,
    ),
    registration => $services,
    ...
    {
        command_id => 'login-conflict-1',
        identifier => 'other',
        password   => 'secret-password',
    }
);
is( $conflicted_login->{status},
    'conflict', 'login rejects a reused command_id for another request' );
