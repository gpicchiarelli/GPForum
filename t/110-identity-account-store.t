...
# A password reset already revoked every session. A change did not, so a user
# who changed their password because they suspected a compromise left every
# other device signed in. See docs/QUALITY_PROGRAM.md 2.4.
#
# Built on its own schema and services: the cases above have already rotated
# this user's password, and a test that depends on where they left it is a test
# that breaks when one of them changes.
my $change_services = GPForum::Test::AccountStoreServices->new(
    credentials => {
        'user-1' => { secret_hash => 'hashed:correct horse battery staple' },
    },
);
my $change_schema = GPForum::Test::Schema->new(
    users => [
        {
            email_normalized => 'giacomo@example.test',
            id               => 'user-1',
...
my $change_store = _account_store( $change_schema, $change_services );
my $changed      = $change_store->change_password(
    {
        current_password => 'correct horse battery staple',
        keep_session_id  => 'session-current',
        new_password     => 'a different sufficiently long password',
        user_id          => 'user-1',
    }
);
ok( $changed->{ok},
    'change_password succeeds with the right current password' );
is( scalar @{ $change_services->revokes },
    1, 'change_password revokes the user sessions' );
is( $change_services->revokes->[0]{user_id},
    'user-1', 'change_password revokes the sessions of that user' );
is( $change_services->revokes->[0]{keep_session_id},
    'session-current',
    'change_password keeps the session the user is typing in' );
is( $change_services->actions->[-1]{action},
    'identity.password.changed', 'the change is recorded as an action' );
