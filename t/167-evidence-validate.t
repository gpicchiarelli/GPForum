...
my $legacy_warn = GPForum::Service::Operations::EvidenceValidate->new->run(
    { paths => ["$legacy_stress"] } );
is( $legacy_warn->{status},
    'degraded', 'legacy stress without redaction markers degrades' );
my $legacy_strict = GPForum::Service::Operations::EvidenceValidate->new->run(
    { paths => ["$legacy_stress"], strict => 1 } );
is( $legacy_strict->{status},
    'fail', 'strict mode fails legacy stress evidence' );
...
my $legacy_drill_warn =
  GPForum::Service::Operations::EvidenceValidate->new->run(
    { paths => ["$legacy_drill"] } );
is( $legacy_drill_warn->{status},
    'degraded', 'legacy staging_drill without meta degrades' );
...
my $lifecycle_ok = GPForum::Service::Operations::EvidenceValidate->new->run(
    { paths => ["$lifecycle"] } );
is( $lifecycle_ok->{status}, 'pass',
    'mail_lifecycle_check evidence validates' );

my $missing =
  GPForum::Service::Operations::EvidenceValidate->new->run( { paths => [] } );
