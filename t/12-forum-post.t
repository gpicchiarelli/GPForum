...
# ADR 0102: a reply without a visibility inherits its thread's effective
# visibility, and may not ask for a broader one.
my %in_members = (
    author_user_id   => 'user-1',
    body_hash        => 'hash-9',
    body_source      => 'Hello members',
    idempotency_key  => 'reply-command-9',
    position         => 3,
    thread_id        => 'thread-1',
    visibility_floor => 'members',
);
is( $composer->prepare( {%in_members} )->{command}{post}{visibility},
    'members', 'a reply inherits its thread\'s visibility' );
is(
    $composer->prepare( { %in_members, visibility => 'public' } )
      ->{errors}{visibility},
    'visibility is broader than its thread',
...
my @row_locks =
  grep { $_->{sql} =~ m/FOR [ ] (?:NO [ ] KEY [ ])? UPDATE/msx }
  @{ $lock_dbh->calls };
is( scalar @row_locks, 1, 'position allocation locks the thread row' );
like(
    $row_locks[0]{sql},
    qr/FOR [ ] NO [ ] KEY [ ] UPDATE/msx,
    'thread row lock uses FOR NO KEY UPDATE, which foreign-key checks pass'
);
like(
    $row_locks[0]{sql},
    qr/locked_at .* moderation_state/msx,
    'thread row lock reads back what the workflow checked'
);
...
# The workflow checks the thread before the transaction; a moderator can lock
# or hide it in between. The store checks again under the thread lock and
# refuses without writing, in the words the workflow's own check uses.
...
_assert_reply_refused(
    {
        ...
        row     => { locked_at => undef, moderation_state => 'hidden' },
    }
);
_assert_reply_refused(
    {
...
sub _assert_reply_refused {
    ...
    my $refusing_schema =
      GPForum::Test::PostStoreLockSchema->new( lock_dbh => $refusing_dbh );
    my $refusing_store = GPForum::Service::Forum::PostStore->new(
        id_service => GPForum::Test::Id->new,
        schema     => $refusing_schema,
    );

    is_deeply(
        $refusing_store->create_post( $case->{command} ),
        { error => $case->{error}, ok => 0 },
        "the store refuses a reply to $case->{label}"
    );
    my @written = grep { scalar @{ $refusing_schema->created_for($_) } }
      qw(AuditLog EventLog OutboxMessage Post PostBody PostRevision
      ThreadCounterShard);
    is_deeply( \@written, [],
        "a refused reply to $case->{label} writes nothing" );
