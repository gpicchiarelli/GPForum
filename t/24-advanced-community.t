...
my $reputation_transactions = $schema->transaction_count;
...
is( $schema->transaction_count - $reputation_transactions,
    1,
    'reputation ledger writes the event and the snapshot in one transaction' );
is_deeply(
    $trust_snapshots->last_find_attrs,
    { for => 'update' },
...
# Projection is one PostgreSQL statement, tested where it runs
# (t/integration/postgres-feed-projection.t). The reader and the removals
# below run over rows put in place directly.
_seed_feed( 'post', 'post-1' );
...
my $feed_cursor =
  encode_base64url('2026-05-23T12:00:00Z|018f1000-0000-7000-8000-000000000002');
...
# Removing a thread with its posts is one statement over PostgreSQL's
# subquery: t/integration/postgres-feed-projection.t.

sub _seed_feed {
    my ( $item_type, $item_id ) = @_;

    for my $user_id (qw(user-1 user-2)) {
        $feed_items->create(
            {
                created_at         => '2026-05-23T12:00:00Z',
                item_id            => $item_id,
                item_type          => $item_type,
                permission_version => $DEFAULT_VERSION,
                rank_score         => $DEFAULT_RANK,
                user_id            => $user_id,
                visibility_version => $DEFAULT_VERSION,
            }
        );
    }
