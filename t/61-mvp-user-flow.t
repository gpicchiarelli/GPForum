...
# The page itself offers the way in: the header's links would satisfy an
# unscoped selector even when the page had none, as it did before 7.2.
$test->get_ok('/bookmarks');
$test->status_is($HTTP_UNAUTHORIZED);
$test->text_is( '#forum-error-heading' => 'Sign in required' );
$test->element_exists(
    'section[aria-labelledby="forum-error-heading"] a[href="/login"]');
$test->element_exists(
    'section[aria-labelledby="forum-error-heading"] a[href="/register"]');
