...
for my $required_file (
    ...
    bin/gpforum-dead-letter-replay
    ...
    script/evidence-validate
    script/evidence-meta
    script/dead-letter-check
    script/dead-letter-replay
    script/search-rebuild
    script/mail-lifecycle-check
    )
  )
{
...
like( $ci, qr/script\/os-preflight [ ] --json/msx, 'CI runs OS preflight' );
like(
    $ci,
...
like(
    $freebsd_rc,
    qr{script/gpforum-carton [ ] exec [ ] hypnotoad [ ] -f [ ]}msx,
'FreeBSD rc starts Hypnotoad through script/gpforum-carton, in the foreground'
);

# 3.4: the rc script could not start a production instance. rc.conf
# variables are never exported, so the configuration never reached the
# application, and daemon(8) tracked a Hypnotoad that had double-forked away.
like(
    $freebsd_rc,
    qr/^[ ]{4}set [ ] -a \n [ ]{4}[.] [ ] "[\$][{]gpforum_env_file[}]"/msx,
    'FreeBSD rc exports the environment file into the service'
);
like(
    $freebsd_rc,
    qr{gpforum_env_file:="/usr/local/etc/gpforum/gpforum[.]env"}msx,
    'from /usr/local/etc/gpforum/gpforum.env by default'
);
like(
    $freebsd_rc,
    qr/must [ ] be [ ] owned [ ] by [ ] root/msx,
    'refusing a file root does not own'
);
like(
    $freebsd_rc,
    qr/must [ ] not [ ] be [ ] readable [ ] by [ ] others/msx,
    'or one others can read'
);
like(
    $freebsd_rc,
    qr/-P [ ] [\$][{]pidfile[}] [ ] -p [ ] [\$][{]manager_pidfile[}]/msx,
    'daemon(8) records its own pid and the manager\'s separately'
);
like(
    $freebsd_rc,
    qr{^procname="/usr/sbin/daemon"}msx,
    'rc tracks the supervisor, which stays up'
);
like(
    $freebsd_rc,
    qr/kill [ ] -QUIT [ ] "[\$][(]cat [ ] "[\$][{]manager_pidfile[}]"[)]"/msx,
    'and stops Hypnotoad gracefully'
);
is( system( 'sh', '-n', 'deploy/freebsd/gpforum' ),
    0, 'the rc script parses as sh' );
...
like(
    path('docs/ops/evidence/2026-09-20-cloud-agent-complete/README.md')->slurp,
    qr/PRIVATE [ ] BETA [ ] NOT [ ] YET/msx,
    'cloud-agent-complete evidence refuses private-beta claim'
);
like(
    path('docs/ops/evidence/2026-09-20-cloud-agent-complete/README.md')->slurp,
    qr/residual_gaps/msx,
    'cloud-agent-complete evidence documents residual_gaps'
);
like(
...
like( path('docs/ops/evidence/2026-09-20-cloud-agent-live/README.md')->slurp,
    qr/residual_gaps/msx, 'cloud-agent-live evidence documents residual_gaps' );
like(
...
like(
    path('script/evidence-validate')->slurp,
